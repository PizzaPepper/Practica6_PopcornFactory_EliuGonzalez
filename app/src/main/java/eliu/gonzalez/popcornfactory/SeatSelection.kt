...
class SeatSelection : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var row1: RadioGroup = findViewById(R.id.row1)
        var row2: RadioGroup = findViewById(R.id.row2)
        var row3: RadioGroup = findViewById(R.id.row3)
        var row4: RadioGroup = findViewById(R.id.row4)
        ...
        confirm.setOnClickListener {
            if (numberSeat != -1) {
                val intent: Intent = Intent(this, FormMovie::class.java)
    ...
    fun checkSeats(Seats: ArrayList<Client>, rows: ArrayList<RadioGroup>) {
        var index: Int = -1
        var subIndex: Int = -1
        for (seat in Seats) {
            index = seat.seat
